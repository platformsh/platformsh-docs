...
* `passthru`: Whether to forward disallowed and missing resources from this location to the application. Can be true, false or a URI path string relative to the web root. In a PHP application, this will typically be the front controller such as `/index.php` or `/app.php`.  If your application relies on a `.htaccess` file with `mod_rewrite` under Apache, this entry is the equivalent. Note: The `passthru` key is only supported when the location key is `"/"`.
