...
* `passthru`: Whether to forward disallowed and missing resources from this location to the application. Can be true, false or a URI path string relative to the web root. In a PHP application, this will typically be the front controller such as `/index.php` or `/app.php`.  If your application relies on a `.htaccess` file with `mod_rewrite` under Apache, this entry is the equivalent. Note: The value of `passthru` is relative to the domain, which means if it does not begin with the same value as the location key it is under the passthru may evaluate to another entry, causing unexpected results. We generally advise ensuring that the `passthru` always begin with the same value as its location string.
