...
Platform.sh supports HTTP caching at the server level. The cache is only applied to `GET` and `HEAD` requests.

The cache must be explicitly enabled using the `cache` key in your `.platform/routes.yaml` file. Platform.sh serves the files or requests generated by the application directly, unless the cache is enabled.
...
To enable the HTTP cache, add a `cache` key to your route in `.platform/routes.yaml`:
...
**Values**
* `true`: enable the cache for this route
* `false`: disable the cache for this route [default]

### `headers`

Adds specific header fields to the cache key, enabling caching of separate responses for those headers.

For example, if the `headers` key is the following, Platform.sh will cache a different response for each value of the `Accept` HTTP request header:
...
Use `Vary: Accept-Encoding` to serve different content depending on the encoding. Useful for ensuring that gzipped content is not served to clients that can read it.
