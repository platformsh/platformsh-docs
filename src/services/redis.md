...
The host name and port number obtained from `PLATFORM_RELATIONSHIPS` would be `applicationcache.internal` and `6379`. Open an [SSH session](/administration/ssh.md) and access the Redis server using the `redis-cli` tool as follows:
