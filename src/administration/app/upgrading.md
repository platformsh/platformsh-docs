...
* The `!archive` tag in YAML files is now deprecated in favor of the more generic [`!include`](/administration/yaml.md).  For example, the following `services.yaml` snippet:
...
* The syntax for the `mounts` key in `.platform.app.yaml` has changed.  Rather than a parsed string, the value of each mount is a [multi-key definition](/administration/app/storage.md).  That is, the following example:
...
in your [web
configuration](https://docs.platform.sh/administration/app-containers.html#locations),
as shown below:
