...
You must have an [SSH key](/gettingstarted/tools.md#ssh) already configured on your account, and have both [Git](/gettingstarted/tools.md#git) and the [Platform.sh CLI](/administration/cli.md) installed before continuing.
...
Platform.sh supports whatever local development environment you wish to use.  There is no dependency on any particular tool so if you already have a local development workflow you're comfortable with you can keep using it without changes.  That's the "[untethered](/administration/local/untethered.md)" option.

For quick changes, you can also run your code locally but use the services hosted on Platform.sh.  That is, your site is "[tethered](/administration/local/tethered.md)" to Platform.sh.  While this approach requires installing less on your system it can be quite slow as all communication with the database or cache server will need to travel from your computer to Platform.sh's servers.

Specific documentation is also available for the [Lando](/administration/local/lando.md) local development toolchain, which supports most applications that Platform.sh supports.
