...
* `--pull-request-clone-parent-data`: Set to `false` to disable cloning of parent environment data when creating a PR environment, so the PR environment starts with no data. (`true` by default)
