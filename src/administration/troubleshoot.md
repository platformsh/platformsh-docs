...
If you see that the processing time of certain requests is slow (e.g. taking more than 1000ms), you may wish to consider using a profiler like [Blackfire](/integrations/blackfire.md) to debug the performance issue.

Otherwise, you may check if the following options are applicable:

* Find the most visited pages and see if they can be cached and/or put behind a CDN.  You may refer to [how caching works](/administration/routes/cache.md).
...
For the most reliable disk usage warnings, we strongly recommend all customers enable [Health notifications](/administration/notifications.md) on all projects.  That will provide you with a push-notification through your choice of channel when the available disk space on any service drops too low.
...
If you're using [MariaDB 10+](/services/mysql.md), you can use the SQL query `SHOW FULL PROCESSLIST \G` to list DB queries waiting for locks.  Find output like the following, and start debugging.
...
Everything will be read-only, except the writable [mounts](/administration/app-containers.md#mounts) you declare.  Writable mounts are there for your data: for file uploads, logs and temporary files. Not for your code.  In order to change code on Platform.sh you have to go through Git.
...
Make sure your repository contains an *index.php* file in the [web location root](/administration/app-containers.md#locations), or that your [Drush](/frameworks/drupal7/drush.md) make files are properly named.
...
1. Use [SSH](/administration/access-site.md) to connect to your environment. Find any long-running cron jobs on the environment by running `ps afx`. Once you have identified the long running process on the environment, kill it with `kill <PID>`. PID stands for the process id showned by `ps afx`.
2. If you're performing "Sync", "Merge", or "Activate" on an environment and the process is stuck, use [SSH](/administration/access-site.md) to connect to the parent environment and identify any long running cron jobs with `ps afx`. Kill the job(s) if you see any.
...
Related documentation: [Build and deploy hooks](https://docs.platform.sh/administration/app/build.html#hooks)
...
Related documentation: [Cron and scheduled tasks](https://docs.platform.sh/administration/app/cron.html#cron-jobs)
