...
For the most reliable disk usage warnings, we strongly recommend all customers enable [Health notifications](/administration/integrations/notifications/) on all projects.  That will provide you with a push-notification through your choice of channel when the available disk space on any service drops too low.
...
Everything will be read-only, except the writable [mounts](/configuration/app/storage/) you declare.  Writable mounts are there for your data: for file uploads, logs and temporary files. Not for your code.  In order to change code on Platform.sh you have to go through Git.
...
Make sure your repository contains an *index.php* file in the [web location root](/configuration/app-containers/#locations), or that your [Drush](/frameworks/drupal7/drush/) make files are properly named.
...
If you see that the processing time of certain requests is slow (e.g. taking more than 1000ms), you may wish to consider using a profiler like [Blackfire](/administration/integrations/blackfire/) to debug the performance issue.
...
1. Use [SSH](/development/access-site/) to connect to your environment. Find any long-running cron jobs or deploy hooks on the environment by running `ps afx`. Once you have identified the long running process on the environment, kill it with `kill <PID>`. PID stands for the process id showned by `ps afx`.
2. If you're performing "Sync" or "Activate" on an environment and the process is stuck, use [SSH](/development/access-site/) to connect to the parent environment and identify any long running cron jobs with `ps afx`. Kill the job(s) if you see any.
