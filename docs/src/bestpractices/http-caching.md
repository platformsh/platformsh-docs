...
The solution is to whitelist the cookies that should impact the cache and include only the application session cookie(s).  For the Router cache see [our documentation]({{< relref "/configuration/routes/cache.md#cookies" >}}).  For other cache systems consult their documentation.
