...
On non-production environments Platform.sh automatically blocks web crawlers using the [X-Robots-Tag header]({{< relref "/administration/web/configure-environment.md#x-robots-tag" >}}).  You can disable that per-environment if needed.
...
In general B&M is not necessary when running on Platform.sh.  Platform.sh's [Backup]({{< relref "/administration/backup-and-restore.md" >}}) functionality offers a faster, more robust and easier to restore backup, and for [exporting data]({{< relref "/tutorials/exporting.md" >}}) using the Platform.sh CLI is just as effective.
...
If that is still insufficient, your site may simply be too large to work effectively with B&M.  We recommend setting up [automated scheduled backups]({{< relref "/administration/backup-and-restore.md#automated-backups" >}}) instead.
