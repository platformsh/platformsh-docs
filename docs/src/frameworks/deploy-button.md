...
To have a deployable template, you need to first prepare the repository.  The Deploy on Platform.sh button will work with any Git repository that is deployable on Platform.sh; that is, it has the necessary [`.platform.app.yaml`](/configuration/app/_index.md), [`.platform/services.yaml`](/configuration/services/_index.md), and [`.platform/routes.yaml`](/configuration/routes/_index.md) files in place.  See the appropriate documentation for how to define those files.
