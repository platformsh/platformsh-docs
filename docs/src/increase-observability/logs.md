...
| `deploy`      | No             | The output of the [`deploy` hook](../configuration/app/hooks/hooks-comparison.md#deploy-hook). Only exists after a `deploy` hook has run. |
...
| `post_deploy` | No             | The output of the [`post_deploy` hook](../configuration/app/hooks/hooks-comparison.md#post-deploy-hook). Only exists after a `post_deploy` hook has run. |
