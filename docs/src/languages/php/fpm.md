...
You can check the maximum number of PHP-FPM workers by opening an [SSH session]({{< relref "/development/ssh.md" >}}) and running following command (example for PHP 7.x):
...
To see how much memory your PHP worker processes are using, you can open an [SSH session]({{< relref "/development/ssh.md" >}}) and look at the PHP access log:
