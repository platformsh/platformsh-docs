...
{{< note >}}
Despite the name, triggering a redeploy will not cause the `deploy` hook to rerun for your application. Both your builds and `deploy` hook are tied to individual commits and are resused until another commit is pushed to the environment. 
