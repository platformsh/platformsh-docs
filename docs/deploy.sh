...
createSearchConfig() {
    ...
    MEILI_TOKEN=$(curl -s -H "Authorization: Bearer $PLATFORM_PROJECT_ENTROPY" -X GET "search.internal/keys" | jq -r '.results[] | select(.description | startswith("Default Search API Key")) | .key')
...
createSearchConfigLocal(){
    ...
    MEILI_TOKEN=$(curl -s -H "Authorization: Bearer $MEILI_MASTER_KEY" -X GET "http://127.0.0.1:7700/keys" | jq -r '.results[] | select(.description | startswith("Default Search API Key")) |.key')
