...
Note that [`deploy` and `post_deploy` hooks](/configuration/app/build.html), as well as [`cron` commands](/configuration/app/cron.html), will run only on the [`web`](/configuration/app/web.html) container, not on workers.

## Inheritance

Any top-level definitions for [`size`](/configuration/app/size.html), [`relationships`](/configuration/app/relationships.html), [`access`](/configuration/app/access.html), [`disk` and `mount`](/configuration/app/storage.html), and [`variables`](/configuration/app/variables.html) will be inherited by every worker, unless overridden explicitly.
