...
the default configuration generated by `{{% vendor/cli %}} project:init` automatically transport the [environment variables for all the services](#service-environment-variables) connected to your app via the `.environment` file.
...
{{% vendor/name %}} exposes the [environment variables](/development/variables/use-variables#use-provided-variables)
about the app and its infrastructure automatically.

Assuming that MySQL, PostgreSQL, and Redis services have been added to your environment, and that the application has been granted access to those services via the following relationships:
...
You can configure email in Laravel by mapping within your `.environment` configuration:
