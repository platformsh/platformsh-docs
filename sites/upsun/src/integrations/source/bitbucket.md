...
| `resources-init` | `default` (production/staging), `minimum` (development) | To [specify a resource initialization strategy](/manage-resources/resource-init.md#environment-branch) for new environments created by the integration. By default, production and staging environments use `default`, and development environments use `minimum`. Using the CLI, pass a single value (`minimum`, `default`, `manual`, or `parent`) to override the strategy for development environments — production and staging always use `default`. To set all three types independently, use the API. `parent` is not available for production. See [available resource initialization strategies](/manage-resources/resource-init.md#specify-a-resource-initialization-strategy). |
