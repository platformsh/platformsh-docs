...
Audit your Vercel setup: 

- Document all add-ons and their configurations 
- List all config vars (`vercel config`)
- Review Profile and identify all process types 
- Check buildpack configuration (`vercel buildpacks`) 
- Document custom domains and SSL certificates
- List folders where your application needs write access
- List users and teams having access to your application
...
3. Adjustments to mounts and storage
   ...
   - Ensures that volumes and file persistence are aligned with how {{% vendor/name %}} manages data across containers.
        ...
        To learn more, refer to these topics: [`mounts`](/create-apps/image-properties/mounts) (the property available in the [single-runtime](/create-apps/image-properties/mounts.md) and [composable](/create-apps/image-properties/mounts.md) images), and [network storage](add-services/network-storage.md).
