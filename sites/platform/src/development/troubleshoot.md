...
- Your app is listening at the wrong place.
  - Check your app's [upstream properties](../create-apps/app-reference/images/builtin-image.md#upstream).
...
To generate anything you need later, [write to disk during a `build` hook](/create-apps/app-reference/images/builtin-image.md#writable-directories-during-build).
Or [declare mounts](/create-apps/app-reference/images/builtin-image.md#mounts),
...
Containers can't be shutdown while long-running [cron jobs and scheduled tasks](/create-apps/app-reference/images/builtin-image.md#crons) are active.
...
For static assets, set cache headers using the `expires` key in your [app configuration](/create-apps/app-reference/images/builtin-image.md#locations).
