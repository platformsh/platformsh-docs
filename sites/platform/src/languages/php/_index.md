...
To serve your app, define what (and how) content should be served by setting the [`locations` parameter](/create-apps/app-reference/images/builtin-image.md#locations).
...
- `passthru` to [define a front controller](../../create-apps/web/php-basic.md#set-different-rules-for-specific-locations) to handle nonexistent files.
  The value is a file path relative to the [app root](/create-apps/app-reference/images/builtin-image.md#root-directory).
...
If you have a `composer.json` file in your code, the default [build flavor is run](/create-apps/app-reference/images/builtin-image.md#build):

```bash
composer --no-ansi --no-interaction install --no-progress --prefer-dist --optimize-autoloader
```

To use Composer 2.x on your project, either use PHP 8.2+ or, in your app configuration, add the following [dependency](/create-apps/app-reference/images/builtin-image.md#dependencies):
...
dependencies:
    php:
        composer/composer: '^2'
```
Adding a dependency to the [dependencies block](/create-apps/app-reference/images/builtin-image.md#dependencies) makes it available globally.
...
See more on [build flavors](/create-apps/app-reference/images/builtin-image.md#build).
...
You can configure your PHP-FPM runtime configuration by specifying the [runtime in your app configuration](/create-apps/app-reference/images/builtin-image.md#runtime).
...
You can provide a custom `php.ini` file at the [app root](/create-apps/app-reference/images/builtin-image.md#root-directory).
...
- The command line interface mode (PHP-CLI) is the mode used for command line scripts and standalone apps.
  This is the mode used when you're logged into your container via SSH, for [crons](/create-apps/app-reference/images/builtin-image.md#crons),
  ...
  where {{<variable "PATH_TO_SCRIPT" >}} is a file path relative to the [app root](/create-apps/app-reference/images/builtin-image.md#root-directory).
...
For more information about the start command, see the [web commands reference](/create-apps/app-reference/images/builtin-image.md#web-commands).
...
```
   ...
   {{<variable "PATH_TO_APP" >}} is a file path relative to the [app root](/create-apps/app-reference/images/builtin-image.md#root-directory).
...
```
   ...
   See more options on how to [configure where requests are sent](/create-apps/app-reference/images/builtin-image.md#upstream).
...
```
   ...
   Both {{<variable "PATH_TO_SCRIPT" >}} and {{<variable "PATH_TO_APP" >}} are file paths relative to the [app root](/create-apps/app-reference/images/builtin-image.md#root-directory).
