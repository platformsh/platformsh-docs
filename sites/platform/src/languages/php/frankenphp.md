...
{{< note theme="warning" title="Worker mode note" version="1" >}}

Note that aplications not explicitly designed for long-running workers may require:

- Manual state resets between requests
- Careful handling of static properties and global variables
- Explicit cleanup logic
- Auditing of caching and service lifecycles

Worker mode offers significant performance gains, but only enable it once you are confident your application can safely operate in a stateful environment.
