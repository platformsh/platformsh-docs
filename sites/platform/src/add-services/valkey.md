...
<table>
    ...
    <tbody>
        <tr>
            <td>{{< image-versions image="valkey" status="deprecated" environment="grid" >}}</td>
            <td>{{< image-versions image="valkey" status="deprecated" environment="dedicated-gen-2" >}}</thd>
        </tr>
    </tbody>
</table>
-->
...
The example above leverages [default endpoint](/create-apps/app-reference/single-runtime-image#relationships) configuration for relationships. That is, it uses default endpoints behind-the-scenes, providing a [relationship](/create-apps/image-properties/relationships.md) (the network address a service is accessible from) that is identical to the name of that service.
