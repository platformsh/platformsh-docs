...
<code>{{< variable "APP_NAME" >}}</code> should match your app's `name` in the [app configuration](/create-apps/app-reference/images/builtin-image.md).
...
Define [app configuration](/create-apps/app-reference/images/builtin-image.md) similar to the following:
...
2. Display the [relationships array](/create-apps/app-reference/images/builtin-image.md#relationships) with `echo ${{< vendor/prefix >}}_RELATIONSHIPS | base64 -d | jq .`,
