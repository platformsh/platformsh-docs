...
* The cron `cmd` syntax is now deprecated in favor of `commands`.
  ...
  For more details, see the [full specification for cron jobs](/create-apps/app-reference/builtin-image.md#crons).
...
Rather than a parsed string, the value of each mount is a [multi-key definition](/create-apps/app-reference/builtin-image.md#mounts).
...
To enable caching on your static files, make sure you include an `expires` key in your [web configuration](/create-apps/app-reference/builtin-image.md), as shown below:
