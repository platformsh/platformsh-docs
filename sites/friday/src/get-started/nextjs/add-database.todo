...
As an example on how to do so, to add a [MariaDB database engine](/add-services/mysql.html) into your Next.js project, complete the following 5 steps:

## 1. Create a new branch for testing
Create a new branch using the following command:
```bash {location="Terminal"}
{{% vendor/cli %}} environment:branch add-mariadb-database
```
...
```bash {location="Terminal"}
{{% vendor/cli %}} merge
 Are you sure you want to merge add-mariadb-database into its parent, main? [Y/n] y
{{% vendor/cli %}} checkout main
git pull {{% vendor/cli %}} main
{{% vendor/cli %}} environment:delete add-mariadb-database
git fetch --prune
```

{{< note >}}
Deploying to production was fast because the image built for the `add-mariadb-database` environment was reused.
{{< /note >}}
