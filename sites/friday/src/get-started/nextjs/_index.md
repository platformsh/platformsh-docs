...
Next.js application needs be built during [build hook](/create-apps/app-reference.html#build-hook). To do so, edit your `.{{% vendor/cli%}}/config.yaml` file with the following:
```yaml {location=".{{% vendor/cli%}}/config.yaml"}
applications:
  app:
